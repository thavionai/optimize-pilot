...
- **Document / skill compression** (`compressDocument`, `compress_document` MCP tool, `/compact-doc` command, CLI `--doc`) — for long instruction docs (SKILL.md, CLAUDE.md, system prompts), drops verbatim and near-verbatim duplicate paragraphs/blocks (keeping the first copy), then applies the prose rules. Headings become block boundaries so a paragraph repeated under different sections still collapses; short blocks are protected by a size gate, and code is never touched. Inspired by the deterministic half of *SkillReducer* (arXiv 2603.29919) — no model call, so a near-duplicate that isn't an exact match is kept rather than guessed at.
